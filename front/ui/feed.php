<div id='feedContainer' style='max-height:0'>   
    <div class='subFrame'>
        <div class='subContent' data-cat="WHAT'S NEW" style='flex-grow:1;display:flex;'>
            <div class='feedWrapper'>
                Quicky check fresh albums and artists : COMING SOON !<br/><br/>
                Register and upload with <b>feedTNZ©</b> companion app : BETA<br/>
                Compare libraries between friends : BETA<br/>
                Add comments on bands / albums : BETA <br/>
                Access streaming platforms from album infos : BETA <br/>
                See what the user is currently listening to ! : COMING SOON !
            </div>
        </div>
    </div>
</div>